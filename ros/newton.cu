...
clock_gettime(CLOCK_REALTIME,&p4_start);
  // using voxel grid filter
// arioshi : 多分スキャン点群全部を走るfor loop 点並列化できないか？
  for (i = 0; i < num; i += inc)
  {
    x = scanptr->x;
    y = scanptr->y;
...
// aritoshi : 点並列ここまで
clock_gettime(CLOCK_REALTIME,&p4_end);

clock_gettime(CLOCK_REALTIME,&p5_start);
  if (gnum > 1)
  {
    identity_matrix6d(H);
...
clock_gettime(CLOCK_REALTIME,&p5_end);
