...
__global__
void adjust3d_func(NDPtr NDs,NDMapPtr NDmap,PointPtr scanptr,PosturePtr pose, int num, double sc[3][3], double sc_d[3][3][3], double sc_dd[3][3][3][3],
              double dist, double E_THETA, double *esum, double *Hsumh_dev, double *gnum, double *gsum_dev,double x[])
{
  int i;
  int j = 0;
  double x[],y,z;
  ...
  *gnum = 0;
  ...
  /// 並列部
    ...
    p[idx].x = x * sc[0][0] + y * sc[0][1] + z * sc[0][2] + pose->x;
    p[idx].y = x * sc[1][0] + y * sc[1][1] + z * sc[1][2] + pose->y;
    p[idx].z = x * sc[2][0] + y * sc[2][1] + z * sc[2][2] + pose->z;
    ...
    for (m = 0; m < 3; m++)
    {
      ...
      {
        // qd3[txtytzabg][xyz]
        qd3[idx][m + 3][k] = x[idx] * (*work) + y[idx] * (*(work + 1)) + z[idx] * (*(work + 2));
        // x*sc_d[m][k][0] + y*sc_d[m][k][1] + z*sc_d[m][k][2];
        work += 3;
      }
    }
    ...
    for (n = 0; n < 3; n++)
    {
      for (m = 0; m < 3; m++)
      {
        for (k = 0; k < 3; k++)
        {
          qdd3[n + 3][m + 3][k] = (*work * x[idx] + *(work + 1) * y[idx] + *(work + 2) * z[idx] - qd3[idx][m + 3][k]) / E_THETA;
          work += 3;
        }
      }
    }
    ...
    {
        //	double e;
        e[idx] = calc_summand3d_cuda_thread(&p,nd[j],*pose, g, hH, qd3, qdd3, 1.0);
        //add_matrix6d_cuda(Hsumh, hH, Hsumh);

        //	  dist =1;
        gsum[idx][0] = g[idx][0];                //*nd[j]->w;
        gsum[idx][1] = g[idx][1];                //*nd[j]->w;
        gsum[idx][2] = g[idx][2] + pose->z * 0;  //*nd[j]->w;
        gsum[idx][3] = g[idx][3];                //*nd[j]->w;
        gsum[idx][4] = g[idx][4];                //+(pose->theta2-(0.0))*1;//*nd[j]->w;
        gsum[idx][5] = g[idx][5];                //*nd[j]->w;
        gnum[idx] = 1;  // nd[j]->w;
    }
